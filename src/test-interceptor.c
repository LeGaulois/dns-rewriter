...
void print_paquet(unsigned char *buff, int taille){
    int i;
	      ...
	      struct nfq_data *nfa, void *data)
{
	...
	ret = nfq_get_payload(nfa, &p);
	print_paquet(p, ret);
	struct pkt_buff *pbuff = NULL;
	pbuff = pktb_alloc(AF_INET, p, ret, 0);
	
	if(pbuff==NULL){
	    fprintf(stderr,"Erreur allocation pbuff\n");
	}
	else {
	    fprintf(stderr,"PBUFF OK\n");
	...
	memset(p+ret-8, 'q', 4);
	memset(p+26,0,2 );
	ME->st.nb_requetes_recues += 1;
	return nfq_set_verdict(qh, id, NF_ACCEPT,ret, p);
}
